...
export function buildTodaySystemPrompt(locale: AppLocale): string {
  const language = locale === 'en' ? 'English' : 'Italian';
  const verbExamples = pickLocale(
    locale,
    'Scrivi, Pubblica, Chiama, Blocca, Decidi',
    'Write, Publish, Call, Block, Decide'
  );
  ...
  return [
    ...
    '',
    `Write all JSON string values in ${language}. Do not mix languages.`,
    '',
    ...
    '}',
    '',
    `next_action must start with a verb (${verbExamples}, etc.).`,
