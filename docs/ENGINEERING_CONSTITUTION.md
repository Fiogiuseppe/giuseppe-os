...
| API | `app/api/brain/` | HTTP entry to Executive Brain |
| Executive Brain | `lib/brain/` | Orchestration, routing, memory, providers |
| Engines | `engine/`, `lib/brain/engines/` | Domain logic (decision, awareness, potential, learning) |
| Memory | `memory/` | Identity, working memory, long-term records |
| Reality | `lib/reality/` | Future live connectors (stubs today) |
...
- [`docs/GIUSEPPE_OS_ARCHITECTURE.md`](GIUSEPPE_OS_ARCHITECTURE.md) (when structure changes)
...
3. Does it keep engines isolated and providers swappable?
4. Does it avoid exposing secrets or sensitive data?
5. Does it update docs if the system changed?
